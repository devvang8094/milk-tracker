...
function Dashboard() {
  ...
  const handleCardClick = async (type) => {
    ...
    const handleEditRecord = (item) => {
      setHistoryOpen(false); // Close modal
      const state = { record: item, editMode: true };

      if (item.source === 'milk') {
        navigate('/add-milk', { state: { ...state, session: item.session } });
      } else if (item.source === 'expense') {
        navigate('/add-expense', { state });
      } else if (item.source === 'withdrawal') {
        navigate('/withdraw', { state });
      }
    };

    const handleDeleteRecord = async (item) => {
      if (!window.confirm("This will update your balance. Are you sure?")) return;

      try {
        if (item.source === 'milk') await deleteMilkRecord(item.id);
        else if (item.source === 'expense') await deleteExpense(item.id);
        else if (item.source === 'withdrawal') await deleteWithdrawal(item.id);

        // Refresh Data
        loadData();
        // Improve UX: Maybe keep modal open? 
        // If we keep modal open, we need to refresh history list too.
        // Let's refetch the specific history we were looking at.
        handleCardClick(historyType);
